...
| **`rimo auth login` が「使用可能な OS キーリングがない」と表示する** | コンテナや Secret Service のないヘッドレス Linux にはブラウザログインのセッションを保存する場所がないため、`rimo auth login` は利用できません。上記の手順でキーを作成し、`RIMO_API_KEY` を設定してください（ローカル保存が不要です）。 |
