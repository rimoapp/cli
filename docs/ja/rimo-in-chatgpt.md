...
3. **新規アプリ（New App）** ダイアログに入力します。
   - **名前（Name）:** `Rimo`
   - **説明（Description）:** `Rimo Voice — ミーティングノート・トランスクリプト・ドキュメントの一覧・閲覧・検索・質問ができます。`
...
**Codex**（ChatGPT のサイドバーにある OpenAI のコーディングエージェント）も、同じホスト型 Rimo サーバーに接続できます。ただしプラグインではなく **MCP サーバー** として追加します。**開発者モード（Developer mode）** はオンにしておく必要があります（上記「[開発者モードをオンにする](#開発者モードをオンにする)」を参照）。
...
4. 保存したら、**Rimo でサインインして組織を承認します** — 上記「[Rimo にサインインする](#rimo-にサインインする)」と同じ、ブラウザでのサインインです。
...
Rimo は、**あなた自身が Rimo で見られるもの** しか ChatGPT に見せません。
...
**存在するはずのノートを ChatGPT が見つけられない。** ChatGPT は、接続した 1 つの組織の中で、あなた自身のアカウントに見えるものしか参照しません。別の組織にあるノートなら、再接続してその組織を選んでください。同僚から共有されていないノートは表示されません。
